...
describe("GoAInput Component", () => {
  ...
  describe("Width Tests", () => {
    ...
    it("width without unit uses CSS custom property", async () => {
      const el = render(GoAInput, {
        testid: "input-test",
        width: "300", // no unit
      });
      ...
      await waitFor(() => {
        const container = el.container.querySelector(".container");
        expect(container?.getAttribute("style")).toContain("--width: 300");
      });
    });
...
it("fractional rem values work correctly", async () => {
    ...
    it("mixed width units work independently", async () => {
      const units = [
        ...
        { width: "15rem", type: "number" },
        { width: "20em", type: "email" },
        { width: "25ch", type: "password" },
        { width: "50%", type: "search" },
      ];
      ...
      for (const { width, type } of units) {
        ...
        await waitFor(() => {
          if (width.includes("ch")) {
            const input = el.getByTestId("input-test");
            const expectedWidth = type === "number" ? "27ch" : "26ch"; // 25 + 2 or 25 + 1
            expect(input.style.width).toBe(expectedWidth);
          } else {
            const container = el.container.querySelector(".container");
